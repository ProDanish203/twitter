...
export const getProfile = async (id: string) => {
    try{
        ...
        if(data){
            return {data, success: true, message: "Profile fetched successfully"}
        }else{
...
export const editProfile = async ({ name, username, image, banner, bio,  pathname}: EditProps) => {
    try{
...
// export const follow = async ({followId, pathname}: followParams) => {
//     try{
//         const session = await getAuthSession()
//         if(!session) 
//             return { success: false, message: "Authentication error"}
...
//         const updateFollowedUser = await User.findByIdAndUpdate(followId, {
